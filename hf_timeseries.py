"""
    1. Implement transformer for time series data using huggingface library:
    https://huggingface.co/blog/time-series-transformers
    2. Create time series datasets: 
...
# load tourism_monthly dataset from Hugging Face Hub. 
# this dataset contains monthly tourism volumes for 366 regions in Australia
...
# convert the start feature of each time series to a pandas Period index
@lru_cache(10_000)
...
# Define InstanceSplitter to sample windows from the dataset for training and validation
...
for epoch in range(40):
    ...
    print(f"Time per epoch: {elapsed:.4f}")
...
forecasts = np.vstack(forecasts) # stack vertically, to get forecasts for all time-series
